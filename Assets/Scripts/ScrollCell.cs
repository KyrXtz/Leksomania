...
public class ScrollCell : MonoBehaviour, ICell
{
    ...
    private void ButtonListener()
    {
        if (_hasToDownload)
        {
            ModalManager.Show("Ούπς!", "Αυτό το επίπεδο δεν είναι διαθέσιμο! Κατέβασμα;", _lvlManager.iconsForModals[0], new[] {new ModalButton(){Text = "Οχι"},
                new ModalButton(){Text = "Ναι",Callback = () =>{
                    var x = new DownloadLevel(_let,_cellIndex);
                } }
                 } ); ;
        ...
        }else if (_notUnlocked)
        {
            ...
            for (int i = 4; i < _let; i++)
            {
                ...
                howManyLeft += c + " επίπεδα " + i + " γραμμάτων\n";
            }
            ModalManager.Show("Ούπς!", "Αυτό το επίπεδο είναι κλειδωμένο!\nΠρέπει να περάσεις ακόμα\n"+howManyLeft, _lvlManager.iconsForModals[0], new[] {new ModalButton(){Text = "Οκ.."}
                 }); ;
        }
        else 
        {
