...
public class LevelManager : MonoBehaviour
{
    ...
    void Start()
    {
     ...
     Debug.unityLogger.logEnabled = false; 
        ...
        //PrefsWrapper.ImportDataFromCloud("{\"test3\":{\"type\":\"string\",\"value\":\"kati\"},\"test2\":{\"type\":\"int\",\"value\":5}}");
    ...
    void Update()
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            {
                ModalManager.Show("Κλείσιμο", "Κλείσιμο εφαρμογής;", iconsForModals[0], new[] {new ModalButton(){Text = "Οχι" } ,
    ...
    {
        var stg = stage + 1;
        LevelTitle.text = "Επόμενο επίπεδο\n" + letters + " Γράμματα: " + stg + "ο";
    }
    ...
    public void revealButton()
    {
        ...
        if (newList.Count > 0) //diaforetika exei brei ola ta grammata
        {
            ...
            ModalManager.Show("Εμφάνιση Γράμματος!", "Μπορείς να εμφανίσεις ένα τυχαίο γράμμα με 10 νομίσματα!\nΜπορείς να διαλέξεις το γράμμα με 20 νομίσματα!",iconsForModals[8], new[] {new ModalButton(){Text = "Άκυρο" } ,
                new ModalButton() { Text = "Θα διαλέξω!" ,
                Callback = () => { RevealBlockWhenClicked.isAllowedForClick = !RevealBlockWhenClicked.isAllowedForClick; toggleButtons(); } },
                new ModalButton() { Text = "Τυχαίο!" ,
    ...
    void actuallyRevealLetter(List<GameObject> newList)
    {
        if (coins < 10)
        {
            ModalManager.Show("Ούπς!", "Δεν έχεις αρκετά νομίσματα..\nΠάτα το + πάνω αριστέρα για να πάρεις!", iconsForModals[0], new[] {new ModalButton(){Text = "Οκ.." } 
                 });
            return;
        }
        removeLerps();
    ...
    void saveWordAsFoundAndCheckAchievementsAndLeaderboards(string wrd,bool isExtra)
    {
        ...
        if (isExtra)
        {
            ...
            PrefsWrapper.Save();
            //check the achievement and leaderboards
            //TODO leaderboard
        }
    ...
    public void showExtraWordsView(bool goIn)
    {
        ...
        if (goIn)
        {
            ...
            ExtraWordsViewSubtitle.text= "Βρές άλλες " +(10 - count).ToString() + " λέξεις για να κερδίσεις νομίσματα!";
    ...
    {
        LevelSelect.transform.GetChild(1).gameObject.GetComponent<TextMeshProUGUI>().text = tempLet + "ΓΡΑΜΜΑΤΑ";
    ...
    void checkIfAnyLetterUnlocked()
    {
        for (int y = startL; y <= endL; y++)
        {
            ...
            if (isUnlocked)
            {
                ...
                ModalManager.Show("Ωραία!", "Ξεκλείδωσες επίπεδα με " +y+ " γράμματα!\nΠήγαινε στην αρχική σελίδα για να διαλέξεις επίπεδο.", iconsForModals[0], new[] {new ModalButton(){Text = "Τέλεια!"}
                 }); ;
    ...
    public bool ownsThisPlayer(int playerNo,int price)
    {
        if (PrefsWrapper.GetInt("OwnsP" + playerNo) == 1)
        {
            ...
            {
                ModalManager.Show("Αλλαγή", "Άλλαξες σύντροφο!", iconsForModals[10], new[] { new ModalButton() { Text = "Οκ!" } });
            }
            return true;
        }
        else
        {
            ModalManager.Show("Αγορά", "Αυτός ο σύντροφος κάνει " + price + " νομίσματα! Συνέχεια;", iconsForModals[0], new[] {new ModalButton(){Text = "Οχι.." } ,
                new ModalButton() { Text = "Ναι!" ,
                Callback = () => {
                    if (coins < price)
                    {
                        ModalManager.Show("Ούπς!", "Δεν έχεις αρκετά νομίσματα..", iconsForModals[0], new[] {new ModalButton(){Text = "Οκ.." }});
                    }
                    else
                    {
    ...
    {
        //if (Intermission.GetBool("IntermissionIn"))
        //{
        //    clearEverything();
        //    disableButtons();
        //    HideAndShow(false);
        ...
        ModalManager.Show("Διακοπή", "Σίγουρα θες να επιστρέψεις στο αρχικό μενού; Η πρόοδος σε αυτό το επίπεδο θα χαθεί.", iconsForModals[0], new[] {new ModalButton(){Text = "Οχι" } ,
                new ModalButton() { Text = "Ναι" ,
                Callback = () => {
                    clearEverything();
                    disableButtons();
